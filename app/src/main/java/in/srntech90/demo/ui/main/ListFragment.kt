    ...
    private fun initMainFrag() {
        ...
        activity?.let {
            ...
            if (movieAdapter == null) {
                ...
                movieAdapter = ListAdapter(it, movieList, object : ICallBackSelector {
                    override fun callDetailView(searchItem: SearchItem, img: ImageView) {
                        (activity as BaseFragmentActivity).callDetailFrag(searchItem, img)
                    }
                })

                mainFragDB.recycleView?.adapter = movieAdapter

                (activity as BaseFragmentActivity).search_badge.setQuery(
                    resources.getString(R.string.app_name),
                    true
                )
                callMovies(
                    (activity as BaseFragmentActivity).search_badge.query.toString(),
                    movieAdapter!!.count
                )
                observeData()
            } else {
